...
const newselfProfileValidator = Joi.object({
  company: Joi.string()
    .min(3)
...
const profileIdValidator = Joi.object({
...
const updateselfProfileValidator = Joi.object({
  profileid: Joi.string()
    .trim(true)
    .min(1)
    .message('ObjectID must be min 1 characters')
    .max(24)
    .message('ObjectID must be max 24 characters')
    .messages({
      'any.required': 'ObjectID is required!',
      'string.empty': `ObjectID Cannot be Empty !`,
      'string.base': `ObjectID must be String !`,
